...
def main(file_path):
    ...
    new_path = "/media/aldupd/UNTITLED 2/light dataset V2"
    ...
    with h5py.File(title, "w") as hdf:
        hdf.create_dataset("depth", data=depth, chunks=None)
        hdf.create_dataset("rel_goalx", data=rel_goalx, chunks=None)
        hdf.create_dataset("rel_goaly", data=rel_goaly, chunks=None)
        hdf.create_dataset("goal_orientation", data=goal_orientation, chunks=None)
        hdf.create_dataset("rel_orientation", data=rel_orientation, chunks=None)
        hdf.create_dataset("GT", data=GT, chunks=None)
